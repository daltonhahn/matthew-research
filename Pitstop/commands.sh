...
sudo docker run -d --name eight --network the-network -p 9098:8088 --env-file eight.env --ip 172.18.0.10 nicholasjackson/fake-service:v0.24.2
sudo docker run -d --name nine --network the-network -p 9099:8089 --env-file nine.env --ip 172.18.0.11 nicholasjackson/fake-service:v0.24.2
sudo docker run -d --name ten --network the-network -p 9100:8090 --env-file ten.env --ip 172.18.0.12 nicholasjackson/fake-service:v0.24.2
sudo docker run -d --name eleven --network the-network -p 9101:8091 --env-file eleven.env --ip 172.18.0.13 nicholasjackson/fake-service:v0.24.2
sudo docker run -d --name twelve --network the-network -p 9102:8092 --env-file twelve.env --ip 172.18.0.14 nicholasjackson/fake-service:v0.24.2
